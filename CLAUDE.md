...
Treat `/Users/elzira/my-proj/ug-analytics` as a separate read-only data dependency. Use the installed `ug-analytics` CLI and inspect `ug-analytics --help` before relying on a command. Never edit the analytics repository from this workspace and never print `UG_COOKIE` or other credentials.

Treat `/Users/elzira/my-proj/work/automations/confluence_tools` as the only Confluence integration. It targets self-hosted `alice.mu.se`; never use Atlassian Cloud MCP. Follow `.claude/skills/ug-product-manager/references/confluence-tool.md`, run `bash scripts/cnfl` from the tool directory, and never expose its `.env` or credentials.
